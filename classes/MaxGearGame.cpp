...
float MaxGearGame::getfDistance()
{
    return fDistance;
}

float MaxGearGame::getfAccTrackCurvature()
{
    return fAccTrackCurvature;
}

float MaxGearGame::getfTargetCurvature()
{
    return fTargetCurvature;
}

float MaxGearGame::getfCurrentLapTime()
{
    return fCurrentLapTime;
}

std::list<float> MaxGearGame::getListLapTimes()
{
    return listLapTimes;
}

Menu MaxGearGame::getMenu()
{
    return menu;
}

void MaxGearGame::setTrackOption(int option)
{
    trackOption = option;
}
...
bool MaxGearGame::OnUserUpdate(float fElapsedTime)
{
    ...
    if((GetKey(Key::UP).bHeld) || (GetKey(Key::SPACE).bHeld) || (GetMouse(Mouse::LEFT).bHeld)){
        car.addTofSpeed(2.0f * fElapsedTime);
    }else{
        car.deductFromfSpeed(1.0f * fElapsedTime);
    }

    //Car Curvature is accumulated left/right input (but inv proportional to speed)
    //To make it harder to turn at high speed
    if((GetKey(Key::LEFT).bHeld) || (ncvDirection == CV_LEFT)){
        car.deductFromfCarCurvature(0.7f * fElapsedTime * (1.0f - car.getfSpeed() / 2.0f));
        car.setnCarDirection(CAR_LEFT);
    }

    if((GetKey(Key::RIGHT).bHeld) || (ncvDirection == CV_RIGHT)){
        car.addTofCarCurvature(0.7f * fElapsedTime * (1.0f - car.getfSpeed() / 2.0f));
        car.setnCarDirection(CAR_RIGHT);
    }

    //If car curvature is too different to track curvature, slow down because player is off track
    ...
    float fTrackCurveDiff = (fTargetCurvature - tracks.getfTrackCurvature()) * fElapsedTime * car.getfSpeed();

    //Accumulate player curvature
    tracks.addTofTrackCurvature(fTrackCurveDiff);

    //Accumulate track curvature
    fAccTrackCurvature += (tracks.getfTrackCurvature()) * fElapsedTime * car.getfSpeed();
    ...
    for(int x = 0; x < ScreenWidth(); x++){
        tracks.setnHillHeight((int)(fabs(sinf(x * 0.01f + fAccTrackCurvature) * 16.0f)));

        for(int y = (ScreenHeight() / 2) - tracks.getnHillHeight(); y < ScreenHeight() / 2; y++){
    ...
    for(int y = 0; y < ScreenHeight() / 2; y++){
        for(int x = 0; x < ScreenWidth(); x++){
            ...
            fRoadWidth *= 0.5f;	//Halve cuz track only goes to 'till middle of screen

            //Depending on where fMiddlePoint is
            float fMiddlePoint = 0.5f + tracks.getfTrackCurvature() * powf((1.0f - fPerspective), 3);
    ...
    car.setnCarPos(ScreenWidth() / 2 + ((int)(ScreenWidth() * car.getfCarPos()) / 2.0) - 7); //Offset for sprite

    //Draw the car representing player actions
    car.drawSprite(this);
    hud.drawHUD(this, fDistance, fTargetCurvature, car.getfCarCurvature(), fAccTrackCurvature, car.getfSpeed(), fCurrentLapTime, listLapTimes);
...
void MaxGearGame::writeRecordToFile()
{
    ...
    if((hud.displayTime(timeRecord) == "-2147483648:-2147483648:-2147483648") || (hud.displayTime(timeRecord) == "0:0:-1")){
        fp << "Recorde: 0:0:0\n";
        fp.close();
        return;
    }
...
void MaxGearGame::drawTransRect(Mat frame, Scalar color, double alpha, Rect region)
{
...
void MaxGearGame::detectAndDraw()
{
    ...
    for(size_t i = 0; i < faces.size(); i++ ){
        ...
        //Detects rectangles' positions and evaluate if they're on the left/right side,
        //assigning it to the cv car control variable 
    ...
    Mat rightArrowOverlay = cv::imread("assets/arrow_right.png", IMREAD_UNCHANGED);
